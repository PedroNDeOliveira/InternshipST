/**
  ...
  ******************************************************************************
  */
#include "app_new_specific_config.h"
...
static int32_t CMW_VD1941_Init(void *io_ctx, CMW_Sensor_Init_t *initSensor)
{
  ...
  config.out_itf.data_rate_in_mps = 1300000000;
#elif defined(CONF_30FPS_900MBPS) || defined(CONF_25FPS_900MBPS)
  config.out_itf.data_rate_in_mps = 900000000;
#elif defined(CONF_25FPS_600MBPS)
  config.out_itf.data_rate_in_mps = 600000000;
#else
...
#if defined(DIRECT_CONNECTION_SWAP_LANE_POLARIZATION)
...
#elif defined (SLIP_RING_CONNECTION_NO_SWAP_LANE_POLARIZATION)
  // HPI debug (no swapping of N and P polarities on clock and L0/L1 lanes)
  config.out_itf.clock_lane_swap_enable = 0;
  config.out_itf.physical_lane_swap_enable[0] = 0;
  config.out_itf.physical_lane_swap_enable[1] = 0;
//#else
//ERROR: direct or slip ring connection selection in app_new_specific_config.h
#endif
...
static int32_t CMW_VD1941_Start(void *io_ctx)
{
  ...
  // HPI toggle CAM_ACT
  ((CMW_VD1941_t *)io_ctx)->EnablePin(0);
  ((CMW_VD1941_t *)io_ctx)->Delay(500);
  ((CMW_VD1941_t *)io_ctx)->EnablePin(1);
...
static void CMW_VD1941_PowerOn(CMW_VD1941_t *io_ctx)
{
  ...
  HAL_Delay(500);   /* NRST signals asserted during 200ms */
  /* De-assert the camera STANDBY pin (active high) */
  io_ctx->ShutdownPin(1);  /* Disable MB1723 2V8 signal  */
	HAL_Delay(500);     /* NRST de-asserted during 100ms */
}
